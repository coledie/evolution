"""
Tools for running series of experiments.

Configuration
-------------
Any single type of configuration can be given at a time to
...
def run(training_loop, filename, log_fn=log):
...
class Series(Module):
    ...
    experiment_params: tuple or list
        Experiment parameter generators, see module docstring.
    ...
    def __init__(
        ...
        for param in experiment_params:
            ...
            if isinstance(first, (int, float)):
                ...
                raise ValueError(f"Failed to recognize type of iterable. {first}")
    ...
    def run(self, n_repeats, log_folder="log"):
        if self.logging:
